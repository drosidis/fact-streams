...
describe('factStore.append()', () => {
  ...
  it('should fail when a race condition would result in duplicate fact sequence for the same stream', async () => {
    ...
    store.onBeforeAppend(async (fact) => {
      await sleep(fact.type === 'received' ? 50 : 10);
      return fact;
    });
    ...
    // Start appending two events in parallel. The first one will complete in 50ms and the second in 10ms, therefore we expect the first one to fail
    ...
    // Wait more than the "sold" sleep period, so that we can check if the event
    await sleep(100);
